...
> **ENVIRONMENT: RpaClaw** — You are running in RpaClaw. There are NO subagents, NO `task` tool, NO `claude` CLI. Do NOT attempt to spawn subagents or call `task`. For eval/testing, saving, and improving skills, follow the **RpaClaw-Specific Instructions** section at the bottom of this file. The "Creating a skill" section below is universal and can be followed as-is. All other sections (Running and evaluating test cases, Improving the skill, Blind comparison, Description Optimization) are Claude Code-specific — **SKIP them entirely** and use the RpaClaw equivalents instead.
...
> **⚠️ RPACLAW: SKIP THIS ENTIRE SECTION.** This section requires subagents and `task` tool which are NOT available. Go to **RpaClaw-Specific Instructions → Eval / Improve Cycle** instead.
...
> **⚠️ RPACLAW: SKIP THIS SECTION.** Go to **RpaClaw-Specific Instructions → Eval / Improve Cycle → Step 5** instead.
...
> **⚠️ RPACLAW: SKIP.** Blind comparison requires subagents which are not available.
...
> **⚠️ RPACLAW: SKIP THIS SECTION.** The automated `run_loop.py` / `claude -p` approach is not available. Go to **RpaClaw-Specific Instructions → Description Optimization** for manual optimization guidance.
...
> **⚠️ RPACLAW: SKIP.** `present_files` and `scripts.package_skill` are not available.
...
## RpaClaw-Specific Instructions

You are running inside the **RpaClaw** environment. The core skill-creation workflow (draft → test → review → improve → repeat) still applies, but with these environment-specific adaptations:

### Environment Constraints

- **No subagents**: You cannot spawn parallel subagents. Run all test cases sequentially, one at a time.
- **No `claude` CLI**: The `claude -p` command is not available. Skip the automated description optimization loop (`run_loop.py`, `run_eval.py`, `improve_description.py`). Instead, iterate on the description manually with the user.
...
1. **Write the skill files** to the session workspace:
   - The workspace directory is provided in the system prompt (e.g., `/home/rpaclaw/<session-id>/`)
   - Create the directory: `<workspace>/skills/<skill-name>/` (or `<workspace>/.agents/skills/<skill-name>/`)
   - The `SKILL.md` MUST include YAML frontmatter with `name` and `description` fields:
     ```
     ---
     name: my-skill
     ...
     ```
   - Write any bundled resources (scripts/, references/, assets/) as needed

2. **Call `propose_skill_save`**: After writing all files, ALWAYS call the `propose_skill_save` tool:
   ```
...
3. **NEVER write directly to `/skills/`** — that directory is **read-only** in the sandbox. Always go through the workspace → propose_skill_save flow.

### Modifying an Existing Skill

When the user asks to modify or improve an existing skill:

1. **Read** the current skill from `/skills/<skill-name>/SKILL.md`
2. **Copy / rewrite** the modified version to the session workspace under `<workspace>/skills/<skill-name>/`
3. Follow the same save workflow above (propose_skill_save → user confirms → system copies to permanent location)

### Eval / Improve Cycle

RpaClaw provides two built-in tools — `eval_skill` and `grade_eval` — that replace manual sequential testing with **independent agent sessions**. Each test runs in a fresh agent that knows nothing about the test design, simulating a real user. The core loop is: **draft → eval → grade → review → improve → re-eval**.
...
1. **Generalize** — don't overfit to the test cases. Fix issues in a way that works for all inputs.
2. **Trim** — remove instructions that didn't help or caused wasted steps.
3. **Explain why** — convey reasoning rather than adding rigid rules.
4. **Bundle repeated work** — if every test run wrote the same helper script, put it in `scripts/`.
...
1. Read → modify → write to workspace
2. Eyeball the change
3. `propose_skill_save`
...
1. **Review the conversation history**: Look at the sequence of tool calls, decisions, and outputs. Identify the core workflow steps.
2. **Generalize**: Replace session-specific values (file paths, query strings, URLs) with parameters or placeholders. Extract the reusable pattern.
3. **Write the SKILL.md**: Structure the workflow as clear instructions with:
...
4. **Save**: Write to workspace → `propose_skill_save`
...
1. Draft a description that covers **what** the skill does AND **when** to use it
2. Make it "pushy" — err on the side of triggering too often rather than too rarely. Example:
   - Weak: `"Create a dashboard"`
   - Strong: `"Create dashboards for data visualization. Use this whenever the user mentions dashboards, charts, metrics display, data overview, reporting, or wants to visualize any kind of data."`
3. Walk through 3-4 realistic prompts mentally: would this description make the agent read the skill? Adjust until it reliably triggers.
