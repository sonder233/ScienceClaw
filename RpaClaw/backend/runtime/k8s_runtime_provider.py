...
class K8sRuntimeProvider:
    ...
    def _resource_name(session_id: str) -> str:
        ...
        return f"rpaclaw-sess-{prefix}-{digest}".rstrip("-")
    ...
    def _build_workspace_volume(self) -> tuple[dict, dict]:
        ...
        mount_path = (
            getattr(self.settings, "k8s_runtime_workspace_mount_path", "/home/rpaclaw")
            or "/home/rpaclaw"
        ).strip()
        pvc_claim = (
    ...
    def _build_pod_manifest(self, name: str, session_id: str, user_id: str) -> dict:
        labels = {
            "app.kubernetes.io/name": "rpaclaw-session-runtime",
            "rpaclaw/runtime": "session",
            "rpaclaw/runtime-name": name,
            "rpaclaw/session-id": session_id,
            "rpaclaw/user-id": user_id,
        }
        labels.update(
    ...
    def _build_service_manifest(self, name: str) -> dict:
        return {
            ...
            "metadata": {"name": name},
            "spec": {
                "selector": {"rpaclaw/runtime-name": name},
                "ports": [
                    {
                        "name": "http",
                        "protocol": "TCP",
