// Generated from Playwright injectedScriptSource.js plus a thin RPA recorder adapter.
// Do not edit the vendored upstream block manually; regenerate from the Playwright runtime instead.
(() => {
  const module = {};
...
(() => {
  if (globalThis.__rpaPlaywrightRecorder)
    return;

  const injectedScript = new (module.exports.InjectedScript())(globalThis, {
    isUnderTest: false,
    sdkLanguage: "python",
  ...
  function makeRoleLocator(match) {
    return {
      method: 'role',
      role: unescapeLiteral(match[1]),
      name: match[2] ? unescapeLiteral(match[2]) : ''
    };
  }

  function makeValueLocator(method, match) {
    return {
      method,
      value: unescapeLiteral(match[1])
    };
  }

  function parseLocatorStep(expression) {
    ...
    while (remaining) {
      ...
      if (!matched)
        return null;
      if (step)
        current = current ? { method: 'nested', parent: current, child: step } : step;
  ...
  function asPlaywrightLocator(selector) {
    const locator = injectedScript.utils.asLocator('python', selector);
    return locator.startsWith('page.') ? locator : 'page.' + locator;
  }

  function visibleCount(elements) {
    let count = 0;
    for (const element of elements) {
      try {
  ...
  function describeKind(playwrightLocator, locatorPayload) {
    if (locatorPayload && locatorPayload.method === 'nth')
      return 'nth';
    if (playwrightLocator.includes('.nth('))
      return 'nth';
    if (playwrightLocator.includes('get_by_role('))
      return 'role';
    if (playwrightLocator.includes('get_by_test_id('))
      return 'testid';
    if (playwrightLocator.includes('get_by_label('))
      return 'label';
    if (playwrightLocator.includes('get_by_placeholder('))
      return 'placeholder';
    if (playwrightLocator.includes('get_by_alt_text('))
      return 'alt';
    if (playwrightLocator.includes('get_by_title('))
      return 'title';
    if (playwrightLocator.includes('get_by_text('))
      return 'text';
    if (playwrightLocator.includes('.locator(') || playwrightLocator.startsWith('page.locator('))
      return 'css';
    return locatorPayload && locatorPayload.method ? locatorPayload.method : 'locator';
  }

  function describeReason(candidate, selected) {
    const prefix = selected ? 'selected Playwright candidate' : 'Playwright candidate';
    if (candidate.strict_match_count === 1)
      return prefix + ' is strict unique';
    if (candidate.strict_match_count === 0)
      return prefix + ' resolves to no elements';
  ...
  function buildCandidate(selector, index, target) {
    ...
    const playwrightLocator = asPlaywrightLocator(selector);
    const locatorPayload = parseLocatorStep(playwrightLocator);
    const candidate = {
      kind: describeKind(playwrightLocator, locatorPayload),
      score: index,
      strict_match_count: matches.length,
    ...
    if (locatorPayload)
      candidate.locator = locatorPayload;
    if (locatorPayload && locatorPayload.method === 'nth')
      candidate.nth = locatorPayload.index;
    candidate.reason = describeReason(candidate, candidate.selected);
    return candidate;
  }

  function buildLocatorBundle(target) {
    ...
    const validation = primaryCandidate ? {
      ...
      details: primaryCandidate.reason,
      selected_candidate_index: 0,
      selected_candidate_kind: primaryCandidate.kind || ''
    } : {
      status: 'broken',
      details: 'Playwright selector generation returned no candidates',
      selected_candidate_index: -1,
      selected_candidate_kind: ''
    };
    return {
      primary: primaryCandidate && primaryCandidate.locator ? primaryCandidate.locator : null,
      candidates,
      validation
    };
  }
  ...
  function getAccessibleName(element) {
    return injectedScript.utils.getElementAccessibleName(element, false) || '';
  }

  globalThis.__rpaPlaywrightRecorder = {
    buildLocatorBundle,
    getRole,
    getAccessibleName,
    parseLocatorStep,
