...
class TraceSkillCompiler:
    ...
    def _render_side_effect_interaction(
        *,
        ...
        if download_signal:
            ...
            TraceSkillCompiler._append_download_save_lines(lines, result_key, download_name)
        lines.append("    await current_page.wait_for_timeout(500)")
        return lines
    ...
    def _render_embedded_ai_code_trace(
        ...
        if download_signal and self._download_trigger_mode(download_signal) == "export_task":
            ...
            download_key = str(download_signal.get("result_key") or download_result_key(download_name))
            ...
            lines.append("    _dl = await _dl_info.value")
            self._append_download_save_lines(lines, download_key, download_name)
        else:
            download_key = ""
            lines.append("    _result = await run(current_page, _results)")
